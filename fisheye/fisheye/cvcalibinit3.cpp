				...
				for(int l = 0; l < 4; l++)
				{
					if(((output_quads[k])->corners[l]->row == i) && ((output_quads[k])->corners[l]->column == j) )
					{
						...
						if( iter == 2)
						{
							...
							cornersY << " ";
#ifdef JIAN
							ptl.push_back(cv::Point2f((output_quads[k])->corners[l]->pt));
#endif
							corner_count++;
						}
...
#ifdef JIAN
	ptl.resize(maxPattern_sizeRow*maxPattern_sizeColumn);
	cv::Mat(ptl).reshape(2, maxPattern_sizeRow).copyTo(outCorners);
#endif
