...
time (&start);
	...
	bool tooClose = false;
	int noDataPtCnt =0;
        int imagePxCnt = depth.rows*depth.cols;
	for (int m=0; m< depth.rows; m++) {
	  if (tooClose == true) break; 
	  for (int n=0; n< depth.cols; n++) {
             //break;
	    unsigned short d = depth.ptr<unsigned short>(m)[n];
	    if (d==0) noDataPtCnt++;
	    if (d >0 && d<600) {
		printf("r %d, c %d, distance: %d\n", m, n, d);
              tooClose = true;
              break;
            }
	    if (noDataPtCnt*4>imagePxCnt) {
		printf("Too many unmeaurable: %d / %d\n", noDataPtCnt, imagePxCnt);
              tooClose = true;
              break;
            }
	...
	if (tooClose == true) printf("			==tooClose==		\n");
    }
  }
